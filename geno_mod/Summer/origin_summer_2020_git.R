...
#### Parents Pairs
dhSP1<-read_excel("/repos/ELBFA_GS_modules/geno_mod/Files_PCM/List of testers of PCM1 by season by year_InBID.xlsx",sheet = 2)
...
load("/mnt/DH_processing/sql_invbid_summer2020.Rdata")
load("/mnt/DH_processing/sql_pedigree_name_summer2020.Rdata")
...
ids_par<-function(miss_inv5,inbredAll,inbredAll2,inb,dhSP1){
  ...
  if((dim(miss3)[1] >0) == TRUE){
    write.csv(miss3,file="/mnt/DH_processing/missing_2020_parents_summer.csv",row.names = FALSE,quote = FALSE)
  }else{
...
finalFileName <- "/mnt/Y3_SummerSeasonDHSet2020_SynthHybrids.tab"
...
geno_format<-function(file){  
  ...
  hybrid_geno <- read.csv(file = "/mnt/Y3_SummerSeasonDHSet2020_SynthHybrids.tab", sep="\t", header=T)
...
dim(geno_wide_ped)
summer_20_train<-geno_wide_ped
save(summer_20_train,file="summer_20_traincuc.Rdata")

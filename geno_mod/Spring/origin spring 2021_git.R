...
dhSP3<-dhSP3%>%dplyr::filter(Year=="2021")
...
#save(Allinbred1,file="/mnt/DH_processing/sql_pedigree_name_spring2021.Rdata")
...
#save(Allinbred3,file="/mnt/DH_processing/sql_invbid_spring2021.Rdata")
...
load("/mnt/DH_processing/sql_invbid_spring2021.Rdata")
load("/mnt/DH_processing/sql_pedigree_name_spring2021.Rdata")
############################
#### input geno automat ####
############################
miss_inv5=miss_inv5
...
ids_par<-function(miss_inv5,inbredAll,inbredAll2,inb,dhSP1){
  ...
  if((dim(miss1)[1] >0) == TRUE){
    write.csv(miss1,file="/mnt/DH_processing/missing_2021_parents_spring.csv",row.names = FALSE,quote = FALSE)
  }else{
  ...
  save(Dh_spr_1_1,file = "Spring_2021_geno_traincuc.Rdata")
...
finalFileName <- "/mnt/Y3_SpringSeasonDHSet2021_SynthHybrids.tab"
...
geno_format<-function(file){  
  ...
  hybrid_geno <- read.csv(file = "/mnt/Y3_SpringSeasonDHSet2021_SynthHybrids.tab", sep="\t", header=T)
...
dim(geno_wide_ped_2)
spring_21_train<-geno_wide_ped_2
save(spring_21_train,file="spring_21_traincuc.Rdata")

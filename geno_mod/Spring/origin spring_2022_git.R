...
#### sql
#load("/mnt/DH_processing/PCM1_Spring2022_inbreds_pedigree.Rdata")
#inbredAll2=inbredAll
#load("/mnt/DH_processing/PCM1_Spring2022_inbreds.Rdata")


### new file from Axel with additional hybrids P1-P3
#Allinbred1=ped_name_sql(invs1)
#Allinbred3=bar_inv_sql(invs)
#save(Allinbred3,file="/mnt/DH_processing/sql_invbid_spring2022.Rdata")
#save(Allinbred1,file="/mnt/DH_processing/sql_pedigree_name_spring2022.Rdata")

load("/mnt/DH_processing/sql_invbid_spring2022.Rdata")#Allinbred 1
load("/mnt/DH_processing/sql_pedigree_name_spring2022.Rdata")#Allinbred 3
...
#### translating DH0 genos to DH1 
inb2=inb[grep("JG1",inb$Pedigree),]
miss_inv1=data.frame(id=unique(inb2$Pedigree))
miss_inv2=data.frame(id=unique(inb2$ProgenyGermID))
Allinbred4=ped_name_sql(miss_inv1)
save(Allinbred4,file="/mnt/DH_processing/sql_pedname_genofile4spring2022.Rdata")

load("/mnt/DH_processing/sql_pednameJG1_genofile4spring2022.Rdata")

#### missing pedigree with JG
#tst=unique(miss_inv1$id)[which(!unique(miss_inv1$id)%in%Allinbred4$INPUT_PEDIGREE_NAME)]
#miss_inv4=data.frame(id=tst)
#tst2=inb%>%dplyr::filter(Pedigree%in%miss_inv4$id)%>%select(ProgenyGermID)%>%mutate(id=ProgenyGermID)

##################
#### ANALYSES ####
##################

## Spring Cucumber (Axel) list
## check the inventory BID of parents with the sql query
dhSP<-read_excel("/mnt/DH_processing/List of hybrids to be tested in Spring 2021-22 (1) (1) (2).xlsx",sheet = "spring 2022")
...
dhSP2<-read_excel("/mnt/DH_processing/List of hybrids to be tested in Spring 2021-22 (1) (1) (2).xlsx",sheet = "spring 2022")
...
#### Parents Pairs
dhSP1<-read_excel("/mnt/DH_processing/List of hybrids to be tested in Spring 2021-22 (1) (1) (2).xlsx",sheet = "spring 2022")
...
ids_par<-function(miss_inv5,inbredAll,inbredAll2,inb,dhSP1){
  ...
  if((dim(miss1)[1] >0) == TRUE){
    write.csv(miss1,file="/mnt/DH_processing/missing_2022_parents_spring.csv",row.names = FALSE,quote = FALSE)
  }else{
  ...
  save(Dh_spr_1_1,file = "Spring_2022_geno_traincuc.Rdata")
  return(Dh_spr_1_1)
}
...
finalFileName <- "/mnt/Y3_SpringSeasonDHSet2022_SynthHybrids.tab"
...
geno_format<-function(file){  
  ...
  hybrid_geno <- read.csv(file = "/mnt/Y3_SpringSeasonDHSet2022_SynthHybrids.tab", sep="\t", header=T)
...
dim(geno_wide_ped)
spring_22_train<-geno_wide_ped
save(spring_22_train,file="spring_22_traincuc.Rdata")

...
source('/repos/ELBFA_GS_modules/Stat_mod/JWAS_Model_git.R')
options(scipen = 999)
...
s3load(object = '/ycao1/ITG/Cucumber/Spring/Cln_cucumber_spring25022022_new.Rdata', 
       bucket = 'veg-apd-sdi-predictiveanalytcs-prod-workspace')

#load("/mnt/Cln_cucumber_spring25022022_new.Rdata")
dataset<-pheno
...
## training set 2020
s3load(object = '/ycao1/ITG/Cucumber/hybrids/spring_20_traincuc.Rdata', 
       bucket = 'veg-apd-sdi-predictiveanalytcs-prod-workspace')
...
## training set 2021
s3load(object = '/ycao1/ITG/Cucumber/hybrids/spring_21_traincuc.Rdata', 
       bucket = 'veg-apd-sdi-predictiveanalytcs-prod-workspace')
...
## training set 2022
s3load(object = '/ycao1/ITG/Cucumber/hybrids/spring_22_traincuc.Rdata', 
       bucket = 'veg-apd-sdi-predictiveanalytcs-prod-workspace')
...
s3load(object = '/ycao1/ITG/Cucumber/inbreds/inbred_geno_cucumber.Rdata', #training inbred_geno
       bucket = 'veg-apd-sdi-predictiveanalytcs-prod-workspace')
s3load(object = '/ycao1/ITG/Cucumber/inbreds/spring_DH_geno_cucumber.Rdata', #2021 summer_DH
       bucket = 'veg-apd-sdi-predictiveanalytcs-prod-workspace')
...
out0 = "/mnt/veg_blup/traintest_spring_2022_1yr/"
...
TRAITS<-c("EXTCO","FRLGT","AFW_C","FRNMK","SHAPE","WFSA_C","NFSA_C","FRNMKAvg_1-84","WFSA_CAvg_1-84","NFSA_CAvg_1-84","NETWTAvg_1-84", "NFSA_CSum_1-84","WFSA_CSum_1-84","NETWTSum_1-84")
dataset_uom<-dataset_uom%>%dplyr::filter(OBSRVTN_REF_CD%in%TRAITS)

### scenario 1 training set 2015-2018 + 2020-2021
dataset_uom<-dataset_uom%>%filter(!GROWSEASON%in%c("2015:01","2016:01","2017:01"))
...
#@seas=NULL is prediction with phenotype in model, seas="2022:01" masks the phenotype of the season
#@ gg = NULL for ABLUP
TRAITS<-c("EXTCO")
results <- data.frame()
for (i in 1:length(TRAITS)){
  ...
  method1='SSGBLUP'
  getoutput(out=out0,dataset=dataset_uom,trait = trait_choosen,gg=gg4,n_gen=5,seas=NULL,method=method1)

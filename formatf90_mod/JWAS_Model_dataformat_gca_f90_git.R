...
getoutput_f90 = function(out,dataset,trait,gg,n_gen=5,seas){
  ...
  if (is.null(seas)== TRUE){
    ...
    raw <- raw %>%
      mutate(TRAIT_VALUE = ifelse(PEDIGREE_NAME %in% test, -999, TRAIT_VALUE)) 
  }
  ...
  ########################################
  ### adding non phenotyped with geno to pheno file
  genos$ID=as.character(genos$ID)
  ID_miss=genos%>%dplyr::filter(!ID%in%as.character(phenos_jwas$ID))
  ID_miss=unique(ID_miss$ID)
  df <- data.frame(matrix(ncol=dim(phenos_jwas[,2:ncol(phenos_jwas)])[2], nrow = length(ID_miss)))
  x <- colnames(phenos_jwas[,2:ncol(phenos_jwas)])
  colnames(df) <- x
  pheno_new=data.frame(ID=ID_miss,df,stringsAsFactors=FALSE)
  pheno_new[is.na(pheno_new)] <- -999
  pheno_new$intercept=1
  phenos_jwas= data.frame(lapply(phenos_jwas, function(x) type.convert(as.character(x), as.is = TRUE)),stringsAsFactors=FALSE)
  pheno_new=data.frame(lapply(pheno_new, function(x) type.convert(as.character(x), as.is = TRUE)),stringsAsFactors=FALSE)
  phenos_jwas=rbind(phenos_jwas,pheno_new)
  jlphenodir = paste0(outbox, '/testphenos_jwas.txt')
  write.table(phenos_jwas, file=jlphenodir, quote=F, sep=',', row.names=F, col.names =T)
...
#check pedigree
#all_ped
#all=c(as.character(all_ped$ID),as.character(all_ped$Dam),as.character(all_ped$Sire))
#unique(all)
#table(phenos_jwas$ID%in%all)
  
### adding non phenotyped to pheno file
#genos$ID=as.character(genos$ID)
#ID_miss=genos%>%dplyr::filter(!ID%in%as.character(phenos_jwas$ID))
#ID_miss=unique(ID_miss$ID)
#df <- data.frame(matrix(ncol=dim(phenos_jwas[,2:ncol(phenos_jwas)])[2], nrow = length(ID_miss)))

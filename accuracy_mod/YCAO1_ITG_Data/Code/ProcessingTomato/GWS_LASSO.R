...
#   inner_join(pheno_sub,., by = 'PEDIGREE_NAME')
# 
...
# 
# set.seed(20200424)
# sel_lambda <- cv.glmnet(x_train, y_train, type.measure="mse", alpha = 0.5)
# mdl <- glmnet(x_train, y_train, lambda = sel_lambda$lambda.min)
# cor(y_train, predict(mdl, newx = x_train))
# str(coef.glmnet(mdl))


geno[,3:ncol(geno)] <- geno[,3:ncol(geno)] - 1
# geno_1 <- geno[geno$Pedigree %in% rownames(G_imputed), ]
# geno_1 <- geno_1[match(geno_1$Pedigree, rownames(G_imputed)),]
################
...
for (trt in trtlist){
  ...
  # pheno_geno <- as.data.frame(geno) %>% 
  #   rename(PEDIGREE_NAME = Pedigree) %>% 
  #   inner_join(pheno_df,., by = 'PEDIGREE_NAME')
  ...
  predicted_gt <- data.frame(dEBV = y_test)
  ...
  for (i in 0:10){
    ...
    temp_pred <- predict(temp, s = temp$lambda.min, newx = x_test)

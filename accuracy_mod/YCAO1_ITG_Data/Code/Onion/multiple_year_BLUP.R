...
AllData_1 <- AllData %>% 
  filter(GROWSEASON %in% seasons & SET_OWNER_PROG %in% program & TRIAL_TYPE == trial_type)
...
dat <- AllData_1 %>% 
  filter(OBSRVTN_REF_CD %in% trait[[market]])
dat <- dat[grep(paste(PMC[[market]], collapse = '|'), dat$PMC),]
...
s3write_using(FUN = write.csv, x = GCA_df_origin, 
              object = 'ycao1/OriginSelection/Onion/MultipleYear_BLUP/Yellow_GCA_longformat.csv', 
              bucket = 'veg-apd-sdi-predictiveanalytcs-prod-workspace')

s3write_using(FUN = write.csv, x = SCA_df_origin, 
              object = 'ycao1/OriginSelection/Onion/MultipleYear_BLUP/Yellow_SCA_longformat.csv', 
              bucket = 'veg-apd-sdi-predictiveanalytcs-prod-workspace')
...
s3write_using(FUN = write.csv, x = gca_df_wide, 
              object = 'ycao1/OriginSelection/Onion/MultipleYear_BLUP/Red_GCA_wideformat.csv', 
              bucket = 'veg-apd-sdi-predictiveanalytcs-prod-workspace')

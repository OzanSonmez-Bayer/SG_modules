getoutput = function(method,out,dataset,trait=trt,gg,n_gen=5,seas=NULL,crop){
  #@method= GBLUP,SSGBLUP
  #@origin=os,hp TO ADD
  ...
  #@crop0 : crop name for inbreeding
  #@method0 : "GCA" or "all" for sca and gca
  #@tmp : inbreeding
  ...
  outbox = paste0(out, method1, trait, '_deep')
  ...
  # add P1 and P2
  my_list = getbdat(dataset=dat1, n_gen=5,crop)
  ...
  IDTableName <- paste0(as.name(crop), '/', as.name(crop), '_IDs.RData')
  print(IDTableName)
  ...
  if (is.null(seas)== TRUE){
    ...
    test <- raw %>% dplyr::filter(GROWSEASON == seas) %>% .$PEDIGREE_NAME %>% unique()
    raw <- raw %>%

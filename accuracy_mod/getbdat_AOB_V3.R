...
getbdat = function(dataset, n_gen=5,crop){
...
output<-function(trait,outbox,crop,method,tmp){
  ...
  if (method == "GCA"){
    ped_data<-ped_data%>%dplyr::filter(mdl=="P_GCA")
...
inbred_vals<-function(pheno_1,crop){
  ...
  tmp$inbreeding = A_matrix5$inbreeding
  file1<-paste0("/mnt/5gen_inbreeding_comp_",crop,".Rdata")
  save(tmp,file=file1)
